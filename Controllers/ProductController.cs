...
namespace CuaHangQuanAo.Controllers
{
    public class ProductController : Controller
    {
            ...
            var data = await _db.Items
                ...
                .Select(x => new
                {
                    ...
                    img = x.ProductVariants.Any()
                        ...
                        : Url.Content($"~/Images/{x.ItemsId}.jpg")
                })
                .Take(8)
                .ToListAsync();
        ...
        {
            var query = _db.Items
                .Include(x => x.ProductVariants) // Include variants for image
                .AsNoTracking()
        ...
        public async Task<IActionResult> QuanAo(ProductListVm f)
        {
            ...
            var q = _db.Items
                ...
                .Include(i => i.ProductVariants) // Include variants for image
        ...
        public async Task<IActionResult> GetSizesByColor(int productId, string color)
        {
            try
            {
                var variants = await _productService.GetAvailableVariantsAsync(productId);
        ...
        [HttpGet]
        public async Task<IActionResult> GetColorsBySize(int productId, string size)
        {
            try
            {
                var variants = await _productService.GetAvailableVariantsAsync(productId);
